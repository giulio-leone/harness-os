# Support Workspace Template

This reference workspace shows how to run HarnessOS for case intake, resolution, and customer follow-through on the `support` workload profile.

## What is included

- portable bootstrap scripts and preview-first wrappers under `.harness/`
- local Copilot CLI skill copies under `.github/skills/`
- a reference mission catalog for triage -> resolution -> follow-up
- example workflow metadata using `deadlineAt`, `recipients`, `approvals`, and `externalRefs`
- support-oriented placeholder assets for a case brief and escalation notes
...
- The reference queue uses first-class workflow metadata so case owners, escalation approvals, and ticket/KB refs stay visible at the issue boundary.
