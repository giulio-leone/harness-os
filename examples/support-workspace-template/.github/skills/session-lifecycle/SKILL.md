...
11. Host-routed claims are explicit: `begin_incremental`, `begin_recovery`, and host-aware `next_action` calls must carry `host` plus `hostCapabilities` so workload-class dispatch stays deterministic and explainable.
