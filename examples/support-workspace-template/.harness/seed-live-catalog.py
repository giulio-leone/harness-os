...
def main() -> int:
    ...
    try:
        ...
        for issue in catalog['issues']:
            cursor.execute(
                'INSERT OR REPLACE INTO issues (id, project_id, campaign_id, milestone_id, task, priority, status, size, depends_on, next_best_action, deadline_at, recipients_json, approvals_json, external_refs_json) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
                (
                    issue['id'],
                    project['id'],
                    issue['campaignId'],
                    ...
                    issue.get('deadlineAt'),
                    to_json_array(issue.get('recipients')),
                    to_json_array(issue.get('approvals')),
                    to_json_array(issue.get('externalRefs')),
                ),
            )
