...
| Supervisor tick runtime | Durable tick inputs, host execution hooks, filtered dashboard reads, queue promotion, ready dispatch, assignment execution, decision traces, backoff/stop conditions, and run summaries | `src/contracts/orchestration-contracts.ts`, `src/runtime/orchestration-supervisor.ts`, `src/runtime/orchestration-assignment-runner.ts` |
...
`runOrchestrationSupervisorTick()` is the single-tick runtime boundary. For every tick it validates input, checks an optional external stop file before database reads, resolves project/campaign scope, loads the dashboard view model, applies dashboard filters, and records an `inspect_dashboard` decision. In `dry_run` mode it only reports planned `promote_queue` and `dispatch_ready` decisions. In `execute` mode it requires `dispatch.assignmentRunner`, owns queue promotion, reloads the filtered dashboard, dispatches only visible ready issue ids through the dispatcher with `promoteBeforeDispatch: false`, and runs up to `assignmentRunner.maxAssignmentsPerTick` resulting assignments. Idle, blocked, external-stop, and error outcomes still return schema-valid tick results with explicit stop reasons and backoff hints.
...
`runSymphonyAssignment()` is the assignment execution boundary used by execute-mode supervisor ticks, `harness-agent-runner`, and `harness_symphony(action: "run_assignment")`. It runs the configured command inside the dispatched worktree, sets `HARNESS_EVIDENCE_ROOT`, `HARNESS_TEST_REPORT_PATH`, `HARNESS_E2E_REPORT_PATH`, `HARNESS_SCREENSHOT_PATH`, and `HARNESS_CSQR_SCORECARD_PATH`, requires the command to write the requested proof files, registers produced artifacts, persists run-scoped CSQR-lite scorecards, and closes the session `done` only when all required evidence exists. Command failures, missing proof files, and unsafe evidence-root placement close the session `failed` with diagnostic evidence so active leases are not leaked.
...
`startTurn()` remains the compatibility wrapper that returns the strict turn result. `startTurnWithTelemetry()` is the richer sidecar boundary: it returns the turn result plus event records, telemetry records, a live session snapshot, and an optional continuation recommendation. Continuation is advisory and caller-triggered for clean exits; retryable failures and stalls can emit bounded backoff recommendations without wiring dispatcher retries. Stall detection is intentionally gated to transports that implement the optional `requestWithEvents()` observer contract, so a unary real transport continues to use `turnTimeoutMs` instead of false-positive stall failures. Event-capable transports can reset the stall deadline on progress, approval, user-input, token, and rate-limit events.

The included scripted process adapter is intentionally deterministic test infrastructure: it can emit valid responses, malformed payloads, hangs, transport closes, process exits, normalized Codex error categories, and event timelines without launching a real Codex binary or opening the network. Approval events are auto-approved under the documented high-trust no-human runtime policy; non-terminal user-input events are captured as pending telemetry and count as activity, while terminal `input_required` still fails with `turn_input_required`. Token telemetry uses absolute totals to avoid double counting, rate-limit telemetry keeps latest snapshots by family/name, and retry/backoff telemetry records retry-after overrides and capped exponential backoff. The assignment runner is the separate M10-I5 execution surface that consumes dispatched assignments and command-produced proof files.
...
The v1 orchestration modules are intentionally additive at the database/schema layer. Existing lifecycle contracts remain valid, with one compatible extension: `SessionArtifactReference.id` is optional on input and present on persisted session artifacts returned from begin/resume/recovery. The public package surface exports stable orchestration modules, including supervisor tick/run schemas, assignment runner schemas, `runOrchestrationSupervisorTick()`, `runOrchestrationSupervisor()`, `runSymphonyAssignment()`, Codex runner schemas, `launchCodexAppServerRunner()`, `startTurnWithTelemetry()` envelopes, and deterministic scripted Codex process adapters. The MCP surface keeps the same six-tool catalog and extends the existing `harness_symphony` tool with `run_assignment`, `supervisor_tick`, and `supervisor_run`; the CLI surface exposes bounded polling through `harness-supervisor` and single-assignment execution through `harness-agent-runner`.
...
The matrix requires run-scoped `typecheck_report`, `state_export`, and `csqr_lite_scorecard` artifacts, plus assignment-scoped `test_report`, `e2e_report`, and optional `screenshot` artifacts for every planned assignment. Reference packet assertions verify that those assignment artifacts are produced by the planned subagent, belong to the planned worktree, are covered by passed gates, and have codebase reference coverage. `runSymphonyAssignment()` now owns the command execution boundary for one dispatched assignment, but still refuses to infer success from exit code alone: the configured command must write the proof files to the `HARNESS_*` paths.

The supervisor E2E path now exercises the full no-human control plane through MCP: `compile_plan` and `plan_issues` seed pending work, `supervisor_run` executes bounded inspect/promote/dispatch/run-assignment ticks without any `harness_session(begin)` call from the operator, and `inspect_state` plus `dashboard_view` prove active leases, worktree metadata, evidence packet ids, CSQR-lite scorecard ids, and artifact counts after runner evidence ingestion. Supervisor tick/run results only claim screenshot or test-report evidence when the configured assignment command wrote those files and HarnessOS registered them.
...
- The dispatcher assigns work and claims sessions; execute-mode supervisor ticks now require `dispatch.assignmentRunner` and consume dispatched assignments through the assignment runner surface.
- Worktree metadata, physical creation, hooks, timeout handling, cleanup, runner launch/turn identity, event-capable fake process behavior, continuation/telemetry sidecars, and evidence manifests are typed; invoking the physical and Codex runner adapters remains an explicit host/runner responsibility.
- Evidence packet validation, deterministic reference E2E assertions, supervisor-driven MCP E2E coverage, and command-produced assignment evidence ingestion exist; CI/PR proof collection remains separate future work.
- MCP dispatch records deterministic worktree/branch assignments and evidence metadata, but direct `dispatch_ready` still does not shell out by default. Hosts opt into execution through `run_assignment`, `harness-agent-runner`, or execute-mode supervisor input with `dispatch.assignmentRunner`.
