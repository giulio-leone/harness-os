...
3. **Supervise, Dispatch, or Begin**: Fully agentic hosts can call `runOrchestrationSupervisor()` or `harness-supervisor` for bounded autonomous polling, or `runOrchestrationSupervisorTick()` for one deterministic dry-run/execute tick. MCP hosts can use `harness_symphony(action: "supervisor_run")` for the same bounded loop, including assignment execution when `dispatch.assignmentRunner` is present, or still call `dashboard_view`, `promote_queue`, `dispatch_ready`, and `run_assignment` as separate steps. Single-worker hosts use `harness_session(action: "begin")` to claim one unblocked task. In all claim paths, leases are atomically assigned to prevent duplicate work.
