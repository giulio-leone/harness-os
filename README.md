...
- `launchCodexAppServerRunner()` defines the opt-in Codex app-server runner boundary with fake-process coverage, `bash -lc` workflow command execution, runner/thread/turn identity, Symphony session-id derivation, read/turn timeout normalization, typed startup/turn error categories, and an additive `startTurnWithTelemetry()` envelope for event-capable transports with continuation recommendations, non-stall approval/user-input events, stall detection, token/rate-limit telemetry, and retry/backoff evidence.
